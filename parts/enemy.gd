...
@onready var predictBall = get_node("../map/Area2D2").position
...
func _physics_process(delta):
	print(Game.directionY)
	var enRango = false
	if reachD or reachI:
		enRango = true
	if enRango:
		hit()
	elif Game.side == 1 and  Game.directionY == -1 :
		if canPredict:
			predictBall = ball._predict()
			canPredict = false
			toPredict.start()
	elif Game.side == -1:
			predictBall = get_node("../map/Area2D2").position
			
	var y_dist = sqrt(pow(predictBall.y - position.y, 30)) 
	var x_dist = sqrt(pow(predictBall.x - position.x, 54)) 
